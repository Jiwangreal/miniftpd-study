...
static void do_stat(session_t *sess)
{
	...
	else if (sess->bw_upload_rate_max > 0)
	{
		...
		sprintf(text,
			...
			sess->bw_upload_rate_max);//格式化字符串
		writen(sess->ctrl_fd, text, strlen(text));
	}
...
static void do_site_chmod(session_t *sess, char *chmod_arg)
{
	...
	char perm[100] = {0};//权限
	char file[100] = {0};//文件名称
	str_split(chmod_arg , perm, file, ' ');
	if (strlen(file) == 0)//说明只有1个参数。文件名称少了
	{
		ftp_reply(sess, FTP_BADCMD, "SITE CHMOD needs 2 arguments.");
		return;
	}
	...
	//更改文件权限的操作
	unsigned int mode = str_octal_to_uint(perm);//字符串转换为8进制的整数
	//man 2 chmod
...
static void do_site_umask(session_t *sess, char *umask_arg)
{
	...
	{
		unsigned int um = str_octal_to_uint(umask_arg);//字符串转换为8进制的整数
		umask(um);//man 2 umask
		char text[1024] = {0};
