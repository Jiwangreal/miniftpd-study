...
{
	/*
	因为现在listen socket由nobody进程来创建，ftp服务进程所对应的socket总是等于-1
	所以需要向nobody进程发送一个命令请求，看看sess->pasv_listen_fd这个socket是否为-1
...
int get_pasv_fd(session_t *sess)
{
	...
	else if (res == PRIV_SOCK_RESULT_OK)//成功应答，回传数据socket
	{
		sess->data_fd = priv_sock_recv_fd(sess->child_fd);
	}
