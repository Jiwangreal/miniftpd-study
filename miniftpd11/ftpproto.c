...
int port_active(session_t *sess)
{
	if (sess->port_addr)//说明PORT模式是激活状态
	{
		if (pasv_active(sess))//还需要判断PASV模式，是否是处于激活状态，若是激活状态，是不允许的
		{
...
//printf("bbbbbbbbbbbbbbbb\n");
	...
	if (pasv_active(sess))
	{
		...
		sess->data_fd = fd;//close(sess->pasv_listen_fd);：此时，监听fd就没有存在的价值了，因为数据连接socket都已经创建好了
	}
...
static void do_pasv(session_t *sess)
{
	...
	//getpeername(已连接socket，addr)，可以获取对端的地址信息
	//getsockname获取本地的地址信息，保存在addr中
	...
	unsigned int v[4];
	sscanf(ip, "%u.%u.%u.%u", &v[0], &v[1], &v[2], &v[3]);//sscanf将ip格式化
	char text[1024] = {0};
	sprintf(text, "Entering Passive Mode (%u,%u,%u,%u,%u,%u).", //sprintf格式化文本信息
		v[0], v[1], v[2], v[3], port>>8, port&0xFF);//port>>8：获取端口的高8位，port&0xFF：低8位，就与上0xFF
