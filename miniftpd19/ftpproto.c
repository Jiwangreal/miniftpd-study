...
session_t *p_sess;//在main.c中声明了这个全局变量了

//断开与客户端之间的连接
//断开连接，不能直接将控制连接关闭掉，因为关闭掉就不能给客户端一个421的应答
//shutdown方法可以进行半关闭
void handle_alarm_timeout(int sig)
{
	shutdown(p_sess->ctrl_fd, SHUT_RD);//先关闭SHUT_RD读端
	ftp_reply(p_sess, FTP_IDLE_TIMEOUT, "Timeout.");
	shutdown(p_sess->ctrl_fd, SHUT_WR);//再关闭写入端
	exit(EXIT_FAILURE);//ftp服务进程退出，nobody进程也会跟着退出
						//因为父子进程通信通过socketpair，子进程的socketpair关闭，会导致父进程的socketpair也关闭
}						//那么priv_sock_get_cmd中readn会返回0，返回0导致nobody进程退出

void handle_sigalrm(int sig)
{
	if (!p_sess->data_process)//当前没有传输数据的状态的话
	{
...
void start_data_alarm(void)
{
	...
	else if (tunable_idle_session_timeout > 0)//tunable_idle_session_timeout表示先前安装了闹钟信号
	{
		// 关闭先前安装的闹钟
		alarm(0);
	}
