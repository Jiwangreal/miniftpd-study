...
*/
	...
	s_ip_count_hash = hash_alloc(256, hash_func);//桶的个数是256
	s_pid_ip_hash = hash_alloc(256, hash_func);//key是pid，因为pid也是整数，所以可以用和ip和对应连接数的哈希表
											//用同一个哈希函数
	...
	while (1)
	{
		conn = accept_timeout(listenfd, &addr, 0);//addr：得到当前连接过来的客户端的ip地址，保存在addr
		if (conn == -1)
			ERR_EXIT("accept_tinmeout");

		unsigned int ip = addr.sin_addr.s_addr;//ip是32bit的整数，所以用unsigned int
		...
		else
		{
			hash_add_entry(s_pid_ip_hash, &pid, sizeof(pid),
				&ip, sizeof(unsigned int));//关键码是pid，pid的大小，值value是ip，ip的大小
...
void handle_sigchld(int sig)
{
	...
	pid_t pid;
	while ((pid = waitpid(-1, NULL, WNOHANG)) > 0)//这里只直到哪个进程退出了，但是不知道哪个ip退出了，即：想知道这个
	{											//pid所连接的客户端对应的ip是多少？
		--s_children;							//所以可以维护一个进程号与ip的哈希表，可以通过进程号得到ip，通过ip可以从
												//ip与连接数限制的哈希表中取到该ip当前的连接数，并且对连接数进行-1操作
		//每次等到一个pid，就到pid与ip对应的哈希表中去查找一下ip
		...
		drop_ip_count(ip);
		hash_free_entry(s_pid_ip_hash, &pid, sizeof(pid));//该进程退出了，那么该进程对应的pid与ip的表项就没有存在的价值了，将其移除
	}

}

//哈希函数，哈希函数的关键码是ip，ip可以是字符串，也可以是整数，当前将整数放进来吧
//因而该哈希函数是整数的哈希函数
unsigned int hash_func(unsigned int buckets, void *key)
{
	unsigned int *number = (unsigned int*)key;

	return (*number) % buckets;//除留余数法得到哈希地址，映射到表空间中，而不会超过表的长度
}
...
unsigned int handle_ip_count(void *ip)
{
	...
	unsigned int *p_count = (unsigned int *)hash_lookup_entry(s_ip_count_hash,
		ip, sizeof(unsigned int));//sizeof(unsigned int)是关键码的大小，就是32bit的整数，不考虑IPv6
									//因为放进去的是整数的指针ip，所以hash_lookup_entry函数强转为unsigned int *的指针
	if (p_count == NULL)//表示第一次连接
	{
		count = 1;
		hash_add_entry(s_ip_count_hash, ip, sizeof(unsigned int),
			&count, sizeof(unsigned int));//关键码key是ip，ip的大小是4个字节，value值是一个整数count，4个字节
	}
	else
	{
		count = *p_count;//取出当前的连接数
		++count;//++操作后，把它放进去
		*p_count = count;
	}

	return count;//返回当前ip所对应的连接数
}

//drop_ip_count函数与handle_ip_count是差不多的，为了连接数-1
void drop_ip_count(void *ip)
{
