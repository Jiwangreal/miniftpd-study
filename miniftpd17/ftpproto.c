...
void upload_common(session_t *sess, int is_append)
{
	...
	long long offset = sess->restart_pos;//保存断点
	sess->restart_pos = 0;

	// 打开文件
	int fd = open(sess->arg, O_CREAT | O_WRONLY, 0666);//给一个权限。实际上权限还收到umask的限制，tunable_local_umask
	if (fd == -1)
	{
	...
	int ret;
	// 加写锁
	//上传文件加一把写锁，上传文件的时候不允许别人读，也不允许别人写这个文件
	...
	// 断点续传方式1：REST+STOR
	// 断点续传方式2：APPE
	//!is_append说明是上传和断点续传方式1
	if (!is_append && offset == 0)		//说明是STOR方式。这不是断点续传，可能是覆盖模式
	{
		ftruncate(fd, 0);//将文件长度清0
		if (lseek(fd, 0, SEEK_SET) < 0)//文件定位到文件头
		{
			ftp_reply(sess, FTP_UPLOADFAIL, "Could not create file.");
			return;
		}
	}
	else if (!is_append && offset != 0)		//说明是REST+STOR
	{
		if (lseek(fd, offset, SEEK_SET) < 0)//将文件偏移到offset位置
		{
			ftp_reply(sess, FTP_UPLOADFAIL, "Could not create file.");
			return;
		}
	}
	else if (is_append)				// APPE，is_append模式表示追加
	{
		if (lseek(fd, 0, SEEK_END) < 0)//文件指针偏移到末尾
		{
			ftp_reply(sess, FTP_UPLOADFAIL, "Could not create file.");
			return;
		}
	...
	struct stat sbuf;
	ret = fstat(fd, &sbuf);
	if (!S_ISREG(sbuf.st_mode))//获取一下文件状态
	{
		ftp_reply(sess, FTP_UPLOADFAIL, "Could not create file.");
		return;
	}
...
static void do_stor(session_t *sess)
{
	upload_common(sess, 0);//0和1的区别是，是否是APPE方式
}
