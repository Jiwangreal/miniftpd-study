...
void upload_common(session_t *sess, int is_append)
{
	...
	while (1)
	{
		...
		//限速会睡眠，睡眠过程中是不会收到abor命令的，所以接下来可以判定一个是否收到了ABOR命令
		limit_rate(sess, ret, 1);
		//当前数据传输还没有完毕，收到了ABOR命令则break
		if (sess->abor_received)
		{
			flag = 2;
			break;
	...
	//如果所有数据都接收完毕，此时ABOR命令过来了，还应该给客户端一个226的应答
	check_abor(sess);
	// 重新开启控制连接通道闹钟
	start_cmdio_alarm();
}
...
static void do_pass(session_t *sess)
{
	...
	//什么时候能接收带外数据呢？需要我们在接收到SIGURG信号之后才可以接收带外数据
	//这里是能够开启接收SIGURG信号的功能
	//可以在登录之后开启接收SIGURG信号的能力
...
static void do_quit(session_t *sess)
{
	...
	exit(EXIT_SUCCESS);//服务端给客户端一个221响应后，需要退出当前进程
}
...
//150 Opening BINARY mode data connection for /home/jjl/tmp/echocli.c (1085 bytes).
	...
	*/
	...
	//为了防止2次226的数据传输完毕的应答，若flag=0以及没有收到abor_received命令的时候，给一个226的应答
	if (flag == 0 && !sess->abor_received)
	{
		// 226
		ftp_reply(sess, FTP_TRANSFEROK, "Transfer complete.");
...
//不在上传和下载过程中，则应该在控制连接通道正常的模式接收的，会在命令映射里面
static void do_abor(session_t *sess)
{
