...
static struct timeval s_curr_time;//方便get_time_sec和get_time_usec同时使用，分开写会多一次系统调用
long get_time_sec(void)
{
	if (gettimeofday(&s_curr_time, NULL) < 0)//NULL表示选择系统当前时区
	{
...
void nano_sleep(double seconds)
{
	...
	//为啥强制转化为long？建议看man nanosleep
	ts.tv_nsec = (long)(fractional * (double)1000000000);//需要纳秒，1s=10亿纳秒，亿的话是8个0
	...
	while (ret == -1 && errno == EINTR);//可能会被信号中断，所以用while，剩余时间返回在nanosleep的第2个变量中
										//也保存在同一个变量ts中了，这样就可以睡眠一定的时间
										//循环睡眠，直到睡眠够了
}
