...
void limit_rate(session_t *sess, int bytes_transfered, int is_upload)
{
	...
	long curr_sec = get_time_sec();//再次获取传输时间
	long curr_usec = get_time_usec();

	//传输bytes_transfered这么多字节数，所流过的时间
	double elapsed;//elapsed为当前传输时间
	elapsed = curr_sec - sess->bw_transfer_start_sec;
	elapsed += (double)(curr_usec - sess->bw_transfer_start_usec) / (double)1000000;//将微妙转换为秒，1微妙=1百万分之1秒
	if (elapsed <= 0)
	{
		elapsed = 0.01;
	}
	...
	if (is_upload)//上传的
	{
		if (bw_rate <= sess->bw_upload_rate_max)//当前传输速度小于最大的上传速度了，就不需要限速
		{
			// 不需要限速
			return;
		}
	...
	//下一次要传输的时间，重新更新
	sess->bw_transfer_start_sec = get_time_sec();//更新时间
...
void upload_common(session_t *sess, int is_append)
{
	...
	char buf[1024];
	// char buf[65536];//vsftpd默认使用的缓冲区大小为64K，传输的比较快，miniftp是<100K的，而它的
						//稍微比100K大一点
						//所以在miniftp中可以做一个配置项tunable_chunk_size来配置传输量
